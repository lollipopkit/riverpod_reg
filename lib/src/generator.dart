...
class ProvidersFileBuilder extends Builder {
  ...
  Future<void> build(BuildStep buildStep) async {
    ...
    await buildStep.writeAsString(
      AssetId(buildStep.inputId.package, config.genPath),
      output,
    );
  }
  ...
  Future<ProvidersConfig> _loadConfig(BuildStep buildStep) async {
    try {
      ...
      return ProvidersConfig(
        ...
        genPath: config?['gen_path'] as String? ?? 'lib/riverpod_reg.dart',
      );
    } catch (e) {
      return ProvidersConfig(
        className: 'MyProviders',
        genPath: 'lib/riverpod_reg.dart',
      );
    }
  }
  ...
  Future<List<ProviderInfo>> _collectProviders(BuildStep buildStep) async {
    ...
    await for (final input in buildStep.findAssets(glob)) {
      ...
      for (final element in library.topLevelElements) {
        ...
        if (element is VariableElement) {
          ...
          if (annotation != null) {
            final providerInfo =
                _createVariableProviderInfo(element, annotation, input.path);
          ...
          if (annotation != null) {
            final providerInfo =
                _createClassProviderInfo(element, annotation, input.path);
            if (providerInfo != null) {
              providers.add(providerInfo);
              imports.add(input.path);
            }
    ...
    for (final provider in providers) {
      provider.importPath = imports.firstWhere(
          (import) => import.contains(provider.sourceFile ?? ''),
          orElse: () => '');
    }
    ...
    for (final metadata in element.metadata) {
      ...
      if (value?.type?.element?.name == 'RegisterProvider') {
        return RegisterProvider(
          ...
          includeNotifier:
              value?.getField('includeNotifier')?.toBoolValue() ?? true,
        );
      }
    }
    return null;
  ...
  ProviderInfo? _createVariableProviderInfo(
      VariableElement element, RegisterProvider annotation, String sourcePath) {
  ...
  ProviderInfo? _createClassProviderInfo(
      ClassElement element, RegisterProvider annotation, String sourcePath) {
  ...
  String? _extractNotifierType(DartType type) {
    ...
    if (typeArgs.isNotEmpty &&
        type.element?.name?.contains('NotifierProvider') == true) {
  ...
  String _generateDisplayName(String providerName) {
    ...
    final name =
        providerName.replaceAll('Provider', '').replaceAll('Notifier', '');
    // Convert to camelCase
    return _camelCase(name);
  }

  String _generateProvidersFile(
      List<ProviderInfo> providers, ProvidersConfig config) {
    ...
    for (final importPath in imports) {
      ...
      final prevDirCount = config.genPath.split('/').length;
    ...
    buffer.writeln(
        '  ${config.className} get providers => ${config.className}(this);');
    buffer.writeln('}');
    buffer.writeln();
    ...
    buffer.writeln();
    buffer.writeln(
        '  Read${config.className} get read => Read${config.className}(ref);');
    buffer.writeln(
        '  Watch${config.className} get watch => Watch${config.className}(ref);');
    buffer.writeln(
        '  UseNotifier${config.className} get use => UseNotifier${config.className}(ref);');
    buffer.writeln('}');
    buffer.writeln();
  ...
  void _generateReadClass(
      StringBuffer buffer, List<ProviderInfo> providers, String className) {
    ...
    for (final provider in providers.where((p) => p.includeRead)) {
      buffer.writeln(
          '  ${provider.returnType} get ${provider.displayName} => ref.read(${provider.providerName});');
    }
  ...
  void _generateWatchClass(
    ...
    for (final provider in providers.where((p) => p.includeWatch)) {
      buffer.writeln(
          '  ${provider.returnType} get ${provider.displayName} => ref.watch(${provider.providerName});');
    }
  ...
  void _generateUseNotifierClass(
      StringBuffer buffer, List<ProviderInfo> providers, String className) {
    ...
    for (final provider in providers
        .where((p) => p.includeNotifier && p.notifierType != null)) {
      buffer.writeln(
          '  ${provider.notifierType} get ${provider.displayName} => ref.read(${provider.providerName}.notifier);');
    }
